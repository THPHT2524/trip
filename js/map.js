...
const Maps = (function () {
  ...
  function cardHtml(s2, i) {
    ...
    /* ★구분 색을 **카드 전체**가 든다 — 테두리가 그 색이다. 지도의 핀도 같은 색이라
       고른 표와 그 핀이 색으로 이어진다(번호에 더해 하나 더).
     ★★못 간 곳은 **색을 뺀다**(2026-09-06). 핀은 이미 그렇게 하고 있었는데
       (.mpin.is-done) 표는 제 구분색 그대로라 둘이 서로 다른 말을 했다 — 지도에는
       회색 핀이 찍혀 있는데 밑의 표는 주황색 식사표였다. --k 하나만 갈면
       테두리·절취선·반원·구분 글자가 한꺼번에 따라온다. */
    const kv = r.done ? 'rail' : U.kvar(r.kind);
    return `<article class="mcard${r.done ? ' is-done' : ''}" data-i="${i}" style="--k: var(--${kv})">
      <!-- ★스텁이 드는 것은 **번호와 구분** — 이 곳이 몇 번째이고 무슨 자리인가.
           둘 다 지도에서 그대로 읽히는 것이라(핀의 수, 핀의 색) 뜯는 쪽에 같이 앉는다.
           몸통에는 언제·어디·얼마만 남는다.
           ★'No.' 이름표는 걷었다(2026-09-06). 표 도안에서 그것이 값을 하는 것은
             일련번호가 000123 처럼 길고 뜻을 모를 때다 — 여기 번호는 한 자리이고
             바로 옆 지도에 같은 수가 찍혀 있어서, 이름표가 없어도 무엇인지 안다. -->
      <span class="mcs">
        <span class="mcn">${s2.n}</span>
        <span class="mck">${esc(r.kind)}</span>
      </span>
      <!-- ★★몸통은 **이름표 붙은 칸 둘**이다(2026-09-06). 표 도안이 값을 그렇게 적는다:
           작은 대문자 이름표를 값 위에 얹고, 칸 사이는 세로 금으로 나눈다.
           이름표를 라틴으로 두는 것은 멋이 아니라 **자간을 줄 수 있어서**다 —
           한글은 자간을 주면 낱자가 흩어진다(이 앱이 여러 번 밟은 함정).
         ★시각이 없어도 칸은 비우지 않는다. 인쇄된 표는 빈 칸을 남기지 않는다 —
      ...
      <span class="mcb">
        <!-- ★단추는 **윗줄 맨 오른쪽**이다(2026-09-06). 옆에 세워 두었더니 이름이
             ...
             ★이름표는 **한글**이다(2026-09-06). TIME·PLACE 로 갔다가 그림(🕘·📍)으로
               갔다가 여기로 왔다. 라틴 대문자는 자간을 줄 수 있어 표다워 보이지만
               이 앱의 나머지가 한글로 말하고, 그림은 뜻이 사람마다 갈린다.
               ⚠ 그래서 **자간을 0 으로 두어야 한다.** 영문 이름표에 주던 .18em 을
                 한글에 그대로 두면 '시 간' 으로 흩어진다(이 앱이 여러 번 밟은 함정). -->
        <span class="mcr">
