...
(function () {
  ...
  function render() {
    ...
    /* ★국가코드 뱃지는 이름 앞에 그대로 선다 — 홈 줄에서 누른 그 뱃지다.
       걷은 것은 **날짜 쪽자판**뿐이다(2026-09-06): 64px 한 줄에 판때기가 열 몇 장
       서니 분장처럼 보였다. 날짜는 판을 벗고 **글자색만** 이어받는다(--route).
     ★국기는 오른쪽 끝, 위아래 가운데. 이름 앞에 작게 달면 윈도우에서 'JP' 두 글자로
       떨어져 글꼴 깨진 것처럼 보이는데, 끝에 크게 두면 떨어져도 국가코드로 읽힌다. */
    ...
    if (t) {
      ...
      const a = t.start_on ? t.start_on.replace(/-/g, '.') : '';
      const dt = (t.start_on && t.end_on) ? `${a} – ${U.md(t.end_on)}` : (a || U.md(t.end_on));
 ...
 /* 머리띠 하나와 그 아래 카드들. 머리띠는 **그 묶음이 얼마였는지**까지 말한다 —
  ...
  /* 여권의 머리 — 이름 한 줄, 종류 한 줄, 그리고 오른쪽에 지나온 것의 띠.
   ...
   ★레일은 양끝으로 삐져나온다 — 여행이 이 카드에서 끝나지 않는다는 뜻이고,
     카드의 미니 레일도 그렇게 생겼다. */
  function pheadHtml() {
    /* ★★이 자리를 네 번 고쳤다: 앱 표식 → 구분 색 점 여섯 → 해마다의 막대 →
       종류·발행국·번호 셋. 넷 다 틀렸고, 틀린 이유가 매번 같았다 —
       **그 자리에 놓을 만한 사실이 없는데 무언가를 놓으려 했다.**
       표식은 아무 말도 안 했고, 범례는 카드에 없는 것을 설명했고, 막대는 여권에
       없는 물건이었고, 마지막 셋 중 둘(TYPE·CODE)은 값처럼 생긴 **상수**였다
       (P 는 늘 P 이고 KOR 는 코드에 박혀 있다). 남은 하나(여행 수)는 3cm 아래
       안내판이 '40 times' 로 이미 말한다.
     ★★그래서 **하나만** 남긴다: 이 기록이 언제부터인가. 여권의 발급일 자리이고,
       이 화면 어디에도 없는 사실이며, 여행이 하나 늘어도 안 바뀌는 값이다 —
       상수가 아니라 **기록의 시작점**이다.
     ⚠ 여행이 없으면 적을 것도 없다(passportHtml 이 그 전에 물러난다). */
  ...
  /* ★국기판은 **늘 두 장**(자리 고정), 이름은 늘 열여섯 칸(칸 폭은 css 가 화면에 맞춰 나눈다).
     ...
     ★세 장은 너무 컸다 — 가로판 셋이 88px, 줄의 28% 였다(2026-09-04 아이폰 실물).
       두 장으로 줄여 58px. **셋째 나라는 윗줄 뱃지에 그대로 있으므로 잃는 것이 없다** —
