 ...
 /* 머리띠 하나와 그 아래 카드들. 머리띠는 **그 묶음이 얼마였는지**까지 말한다 —
  ...
  async function fillFromFlight() {
    ...
    try {
      ...
      /* 기간 — 첫 편이 뜬 날부터 **마지막 편이 내린 날**까지.
         ★집에 닿았을 때만 적도록 걸어 봤다가 풀었다(2026-09-07). 가는 편만 적어
           두면 하루짜리가 되는 건 맞지만, 그건 **아직 안 적은 것**이지 틀린 값이
           아니다 — 오는 편을 적으면 그 자리에서 늘어나고, 안 적어도 사람이 고친다.
           빈 칸으로 두는 쪽이 오히려 뭘 해야 하는지 안 보인다. */
      const first = got[0], last = got[got.length - 1];
      if (first.from.on) $('new-from').value = first.from.on;
      if (last.to.on) $('new-to').value = last.to.on;
