...
const U = (function () {
  ...
  /* ── 공항 이름 ─────────────────────────────────────────────────────────
     ★★**표를 없앴다**(2026-09-08). 예전에는 여기 IATA→한글 표 일흔 줄이 있었는데,
       새로 가는 공항마다 사람이 한 줄씩 심어야 했다 — 푸꾸옥이 그렇게 영어로 들어왔다.
       이제 api/_wikidata.js 가 **코드로 물어서** 한글 이름을 붙여 보내 준다.
       그러니 여기서는 아무것도 안 한다: 피드가 준 name 을 그대로 쓴다.
     ★못 찾으면 영문이 그대로 온다. 여권 지도가 점을 찍는 규칙이 **이름이 '공항' 으로
       끝나는 것**이라 그 줄은 지도에서 빠진다 — 화면에서 한 번 고치면 된다. */
