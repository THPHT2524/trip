...
const Cost = (function () {
  ...
  function draw() {
    ...
    /* 영수증 머리 — **이름을 걷었다**(2026-09-07). '여행 영수증' → '여행 비용' 까지
       갔다가 이름 자체가 군더더기였다는 데로 왔다: 넉 장 중 이 종이와 설정 판만
       제 이름을 머리에 달고 있었고(일정·지도는 안 단다), 아래 탭이 늘 떠 있는데다
       지금 판이 굵게 켜져 있으니 어디 있는지를 세 번째로 말하는 줄이었다.
       ★그 자리에 새 그림을 넣지 않는다. 이 종이에는 **뜯은 자국**이 이미 있고
         (css .rcp::before), 영수증으로 읽히게 하는 일은 그 톱니가 하고 있었다 —
         이름은 톱니가 하는 말을 글자로 한 번 더 적은 것이었다.
       ★남는 줄은 곁말 하나뿐이다. 여행 이름·기간은 바로 위 머리말이 이미 적고,
         이 종이가 제 것으로 말할 것은 **몇 줄인지와 무슨 돈인지** 둘뿐이다.
       ⚠ 셀 것이 없으면 **통째로 비운다**. 빈 <span> 하나를 남기면 .rhd:empty 가
         안 맞아서 점선 한 줄이 허공에 그어진다.
       ⚠ 한글은 **고딕으로 싸서** 낸다(.ko). 이 줄은 등폭에 자간이 있는데 Plex Mono
         에는 한글이 없어서 낱자만 대체 글꼴로 떨어지고 사이의 빈칸은 등폭 그대로다 —
         '26 항 목 · 원 화 기 준' 으로 벌어진다. */
    $('cost-head').innerHTML = paid.length
      ? `<span class="rsb">${paid.length}<i class="ko">항목</i> · <i class="ko">원화기준</i></span>`
      : '';
