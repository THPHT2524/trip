...
const Cost = (function () {
  ...
  function draw() {
    ...
    if (c && c.got > c.bal + 0.5)
      fine.push('현금 지출은 환전 내역의 평균 환율을 기준으로 환산하였습니다.');
    /* ★money.js 가 줄마다 '사람이 적은 환율인가, 그날 고시를 가져다 쓴 것인가' 를
       남겨 둔다 — 가져다 쓴 줄이 하나라도 있을 때만 이 문장이 선다.
       ★★'1회차' 가 아니라 **전신환매도율**이라고 적는다(2026-09-06). 카드 청구는
         승인일이 아니라 **매입일**(보통 1~3일 뒤) 환율로 잡히는데 매입일은 카드사만
         안다 — 그래서 지출한 날로 근사한다. 그 근사값의 출처는 네이버 일별 시세의
         sendValue 이고, 이건 하루에 한 행이라 **시각 개념이 없다**. 1회차라고 적으면
         있지도 않은 정밀도를 말하게 된다. (신한 1회차는 api/more.js 가 더모아용으로
         이미 받아 오고 과거 날짜·통화 46개가 다 되므로, 옮기려면 옮길 수는 있다.) */
    if (paid.some(r => { const p = M.per.get(r.id); return p && p.auto; }))
      fine.push('카드 지출은 지출한 날의 전신환매도율을 적용하여 환산하였습니다.');
