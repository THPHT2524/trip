...
const Cost = (function () {
  ...
  function draw() {
    ...
    /* 영수증 머리 — 가게 이름 자리에 **무슨 종이인지**. 여행 이름은 바로 위
       ...
       ★이름은 **여행 비용**이다(2026-09-07). '여행 영수증' 으로 적어 보니 탭에는
         '비용' 이라 적혀 있어서 한 걸음에 이름이 둘이 됐다. 게다가 영수증은
         이 종이가 **입은 옷**이지 담은 것이 아니다 — 가게 하나가 한 번 끊어 주는
         것이 영수증인데 이건 여행 전체다. 종이는 **담은 것으로** 부른다 —
         옷은 점선·바코드·TOTAL 이 이미 말하고 있으니 글자로 또 적을 일이 아니다.
         그러면 설정 판과 규칙도 같아진다 — **탭 이름 앞에 '여행'**.
       ★이 화면에서 사람의 목소리로 말하는 줄은 여기 하나뿐이라 한글로 둔다.
    ...
    /* ⚠ 머리는 **늘 뜬다**. 항목 수와 기준은 셀 것이 있을 때만 말한다 —
       ...
       '원화기준' 도 할 말이 아니다.
     ★★날짜를 뺐다(2026-09-07). 바로 위 여행 머리말이 '2026.08.29 – 08.31' 이라고
       이미 적고 있어서 한 화면에 같은 기간이 두 번 섰다 — 게다가 하나는 네 자리
       연도, 하나는 두 자리라 같은 값인지도 한 박자 늦게 알아봤다.
       영수증이 제 것으로 말할 것은 **몇 줄인지와 무슨 돈인지** 둘뿐이다.
     ★★그러고 나니 머리가 두 줄뿐이라 허전해져서 **라틴 줄을 넣는다**(2026-09-07).
       영수증 머리가 예뻐 보이는 까닭은 굵은 이름 밑에 **자간 넓은 작은 글자**가
       한 겹 깔리는 짜임이다 — 가게 영수증이 상호 밑에 주소와 전화를 그렇게 찍는다.
       없는 사실을 지어내는 대신, 이 종이가 무엇인지를 그 자리에 적는다.
       설정 판이 이미 같은 짝(여행 설정 / TRIP SETTINGS)이라 두 종이가 맞물린다.
     ⚠ 한글은 **고딕으로 싸서** 낸다. 이 줄은 등폭에 자간 .7px 인데 Plex Mono 에는
       한글이 없어서 낱자만 대체 글꼴로 떨어지고 사이의 빈칸은 등폭 그대로였다 —
       '26 항 목 · 원 화 기 준' 으로 벌어졌다(.ko 주석 참고). */
    $('cost-head').innerHTML = `<span class="rnm">여행 비용</span>
      <span class="ren">Trip expenses</span>
      <span class="rsb">${paid.length
        ? `${paid.length}<i class="ko">항목</i> · <i class="ko">원화기준</i>` : ''}</span>`;
