...
const Cost = (function () {
  ...
  function draw() {
    ...
    /* ★★곁줄이 **머리에 있던 줄을 넘겨받는다**(2026-09-07). '26항목 · 원화기준' 은
       종이의 제목이 아니라 **이 수의 각주**다 — 큰 수 밑에 붙어야 읽을 때 이어진다.
       그러면 못 센 줄과도 한 줄에 선다: 이 수는 26항목을 원화기준으로 센 것이고
       2건은 못 셌다. 한 생각이라 한 줄이다(전에는 머리와 곁줄로 갈라져 있었다).
       ⚠ 못 센 줄은 그 안에서 색으로 갈린다 — 같은 줄에 있어도 경고는 경고다. */
    $('cost-total').innerHTML = paid.length
      ? `<span class="tl">총 지출</span>
         <span class="big">${esc(U.money(total, U.SETTLE))}</span>
         <span class="sub">${paid.length}항목 · 원화기준`
    ...
    /* ★★영수증 머리를 **없앴다**(2026-09-07). 이름을 걷고 나니 남은 것은
       '몇 항목 · 무슨 돈' 한 줄뿐이었는데, 그건 이 종이의 제목이 아니라 바로 밑
       합계의 각주였다 — 합계 곁줄로 내려보내고 머리는 지운다(index.html).
       종이의 첫 인상은 뜯은 자국(css .rcp::before)이 맡는다. */
    ...
    /* ★현금 칩에 담는 것은 **두 짝뿐**이다(2026-09-06). 환전·사용·남음 셋을
       ...
       **바꾼 것과 남은 것**만 남긴다 — 쓴 것은 그 둘의 차이라 셋 중 유일하게
       ...
       ★★이름표 셋을 **두 자로 맞춘다**(2026-09-10). '환전 · 쓴 현금 · 현금 잔액' 이라
         둘·넷·다섯 자였는데, 이름표는 값과 나란히 서는 **기둥 하나**라 길이가 제각각이면
         그 줄이 세 덩이로 흔들린다 — 값 줄은 등폭으로 가지런한데 그 위만 들쭉날쭉했다.
       ⚠ 여기 '잔액' 이 아니라 '현금 잔액' 이라고 적어 두었었다(칩만 보고도 무슨 잔액인지
         알아야 한다는 이유였다). 그 걱정은 **셋이 한 문장이 되면서 풀렸다**: 환전 −
         사용 = 잔액 이 왼쪽부터 그대로 읽히고, 가운데 점선이 그 뺄셈의 부호다.
         낱말 하나만 떼어 읽을 일이 없으니 '현금' 을 앞에 붙일 자리도 없다.
         (무엇의 셈인지는 바로 위 곁말과 아래 깨알글씨가 이미 말한다.)
       ★★가운데 빈자리에 **쓴 것을 점선 위에 얹는다**(2026-09-07). 위에서 '되짚을
       ...
       ⚠⚠ 원화는 **이름표 줄로 올린다**(첫 시도는 접혔다: 293px 에 값 다섯이 안 들어가
         잔액이 둘째 줄로 떨어졌다). 올리고 나면 값 줄에 현지돈 셋만 남아
    ...
    $('cost-wallet').innerHTML = (c && c.bal > 0.5 && c.got > 0) ? `
      ...
      <div class="wallet">
        ...
        <div class="wm"><span class="wt">사용</span>
          <span class="wv"><i class="wd"></i>
            <b class="wf">${esc(U.money(c.got - c.bal, c.cur))}</b>
            <i class="wd"></i></span></div>
        <div class="wc on"><span class="wt">잔액<span class="wk">${krw(c.paid)}</span></span>
    ...
    /* ★줄을 **안 끊는다**. 한 줄에 한 문장씩 세워 봤더니 셋이 나란한 목록처럼 읽혀
       ...
       ★말투는 '하였습니다' 로 맞춘다. 영수증 아랫단의 어투이기도 하고, 둘이 한
         문단으로 이어졌으므로 '제외하였습니다' 옆에 '환산했습니다' 가 서면 어긋난다. */
    /* ★현금은 **오직 환전으로만** 지갑에 들어온다(money.js 의 잔액은 settle==='exchange'
       에서만 는다). 그러니 환전이 없으면 현금 지출도 없다 — 두 문장이 같은 조건 하나에
       묶이는 이유다. 환전을 안 한 종이에서는 '뺐다' 도 '평균 환율' 도 할 말이 없다. */
    const swap = rows.some(r => r.settle === 'exchange' && r.cost != null);
    const fine = [];
    if (swap) fine.push('환전은 지출이 아니므로 합계에서 제외하였습니다.');
    /* ★★환산 규칙은 **한 문장**이다(2026-09-06). 현금과 카드를 따로 세워 두면 둘이
       서로 다른 얘기처럼 읽히는데, 실은 '이 종이의 수를 어떻게 원화로 바꿨나' 하나다.
       ★카드 쪽은 조건을 안 단다. '이 줄은 고시를 가져다 썼다' 는 식으로 켰다 껐다 하면
         종이마다 규칙이 다른 것처럼 보인다 — 규칙은 늘 같고, 어느 쪽이 쓰였는지는
         위의 수가 이미 말한다.
       ★★'1회차' 가 아니라 **전신환매도율**이라고 적는다. 카드 청구는 승인일이 아니라
         **매입일**(보통 1~3일 뒤) 환율로 잡히는데 매입일은 카드사만 안다 — 그래서
         지출한 날로 근사한다. 그 근사값의 출처는 네이버 일별 시세의 sendValue 이고,
         이건 하루에 한 행이라 **시각 개념이 없다**. 1회차라고 적으면 있지도 않은
         정밀도를 말하게 된다. (신한 1회차는 api/more.js 가 더모아용으로 이미 받아
         오고 과거 날짜·통화 46개가 다 되므로, 옮기려면 옮길 수는 있다.) */
    fine.push((swap ? '현금 지출은 환전 내역의 평균 환율을 기준으로 ' : '')
            + '카드 지출은 지출한 날의 전신환매도율을 적용하여 환산하였습니다.');
    /* ★승인금액을 적어 둔 줄은 **환산하지 않았다** — 카드사가 잡은 원화를 그대로 쓴다.
       위 문장만 두면 그 줄까지 고시로 환산한 것처럼 읽힌다(2026-09-06, 승인금액 칸이
       환율 칸을 대신하면서 생긴 구멍이다). */
    if (paid.some(r => r.fx != null))
      fine.push('승인금액이 적힌 줄은 환산하지 않고 그 금액을 그대로 지출로 계산하였습니다.');
    /* 적은 것이 하나도 없는 종이는 셈을 한 적이 없다 — 그때는 규칙도 말하지 않는다 */
    $('cost-fine').innerHTML = paid.length ? esc(fine.join(' ')) : '';
