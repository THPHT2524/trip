...
const DB = (function () {
  ...
  const trips = {
    ...
    /* 여행 목록 화면이 카드마다 '그 여행의 모양' 을 그리려면 일정이 필요하다.
       ★★**줄을 통째로 받던 것을 접었다**(2026-09-17, supabase/home.sql 참고).
         카드가 그리는 것은 셋뿐인데(몇 곳인가 · 그날그날 무슨 구분이었나 · 다녀온 공항)
         그걸 그리려고 내가 속한 **모든 여행의 모든 줄**을 받아 오고 있었다.
       ★지금 데이터로는 181줄이라 아프지 않다. 아픈 것은 자라는 방향이다 — 오사카처럼
         적은 여행이 열 개만 더 생기면 1,000줄을 넘고, Supabase 의 Max rows 기본값이
         1000 이라 넘으면 **오류가 아니라 조용히 잘린다.** 그때 증상은 '오래된 카드부터
         곳 수와 레일이 사라진다' 이고 화면은 아무 말도 안 한다 — 2026-09-02의 id 누락
         사고와 똑같이 생겼다. 여행당 한 줄이면 여행 수만큼만 자란다.
       ★돌려주는 꼴: { trips: [{trip_id, stops, days:[[날짜,[구분…]]]}], air: [공항 줄들] }
         공항 줄만은 낱낱이 온다 — 세계지도가 점과 항로를 그리려면 좌표와 시각이
         줄마다 있어야 해서 접을 수가 없다.
       ★못 받은 것과 없는 것을 **구별해서** 돌려준다(null 대 빈 것). 전에는 둘 다 [] 였는데,
         한 번 실패하면 홈 카드가 조용히 곳 수와 레일을 잃고 통화 코드만 남았다
         (2026-09-02, 마이그레이션 직후 실제로 한 번 그랬다). 부르는 쪽이 판단해야 한다. */
    shape: async () => {
      if (mode() !== 'cloud') return { trips: [], air: [] };
      const { data, error } = await sb.rpc('home_shape');
  ...
  /* ⚠ ref_code·book_url 이 여기 있었다(2026-09-10에 걷었다). 2026-09-01 에 폼에 칸 둘로
     ...
     ⚠ 되살릴 일이 생기면 **셋이 함께 와야 한다**: 표의 칸 · 여기 COLS · shape().
       하나만 있으면 또 이 자리로 돌아온다(shape 만 있던 동안은 늘 null 을 실어 보내서,
       값이 들어 있었더라도 그 줄을 고치는 순간 지워지고 있었다). */
