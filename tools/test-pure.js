...
/* 거래 일시 → 어느 날 고시가 적용되나. 신한 1회차는 아침 8시 20분쯤 나오므로
   9시 전에 긁은 건은 **전날** 고시로 청구된다 — themore 도 같은 선을 긋는다.
   ★하와이·미국에서는 한국시간 새벽에도 가게가 열려 있어 실제로 걸리는 경우다. */
eq(MORE.fxDay('2026-09-03T09:00'), '2026-09-03', '한국시간 9시는 그날 고시');
eq(MORE.fxDay('2026-09-03T08:59'), '2026-09-02', '★9시 1분 전이면 전날 고시다');
eq(MORE.fxDay('2026-09-03T23:59'), '2026-09-03', '밤 늦게도 그날 고시');
eq(MORE.fxDay('2026-09-01T03:00'), '2026-08-31', '달을 넘어가도 하루만 물러난다');
eq(MORE.fxDay('2026-01-01T00:00'), '2025-12-31', '해를 넘어가도 마찬가지');
eq(MORE.fxDay(''), null, '안 적었으면 null');
eq(MORE.fxDay('2026-09-03'), null, '시각이 없으면 null — 9시 전인지 알 수 없다');
eq(MORE.kstNow(Date.UTC(2026, 8, 2, 15, 5)), '2026-09-03T00:05', 'kstNow 는 한국시간이다');

/* ── 통화별 소수점 자릿수 ─────────────────────────────────────────────
   ★**긁어 달라고 말할 수 있는 자리까지만** 낸다. themore 는 ISO 4217 을 따라 TWD·IDR 을
     두 자리로 두지만, 대만 단말은 NT$136.31 을 못 받고 루피아는 소수를 쓰지 않는다 —
     '136.31 긁어 주세요' 는 답이 아니다. 그래서 그 둘만 일부러 다르게 간다.
   ★TWD 는 1달러가 43원쯤이라 정수로 끊으면 999 에 최대 43원 못 미친다(포인트 86 손해).
...
/* ── 그날 안의 차례 ───────────────────────────────────────────────────
   ★차례가 seq 하나로 정해지면서(2026-09-17) 이 셈이 **화면의 순서 그 자체**가 됐다.
     틀리면 줄이 엉뚱한 자리에 앉는데, 그건 눈으로만 보이고 오류로는 안 보인다. */
{
  const day = [
    { id: 'a', seq: 10, at_time: '09:00' },
    { id: 'b', seq: 20, at_time: '12:00' },
    { id: 'c', seq: 30, at_time: null },
  ];
  eq(U.seqAt(day, '10:30'), 15, '★시각을 보고 두 줄 사이에 끼운다');
  eq(U.seqAt(day, '08:00'), 0, '  제일 이른 시각은 맨 앞(첫 줄보다 열 칸 앞)');
  eq(U.seqAt(day, '23:00'), 25, '  늦은 시각은 시각 없는 줄 **앞**에 선다');
  eq(U.seqAt(day, null), 40, '★시각을 안 적으면 맨 뒤 — 정한 줄들 뒤에 선다');
  eq(U.seqAt([], '09:00'), 10, '  빈 날의 첫 줄');
  eq(U.seqAt(day, '09:00'), 15, '  같은 시각이면 그 줄 **뒤** — 같은 시각끼리는 적은 차례가 남는다');

  /* ★사이가 없으면 앞 줄 바로 다음 수를 준다 — 같은 수가 되어도 시각이 가른다 */
  eq(U.seqAt([{ seq: 10, at_time: '09:00' }, { seq: 11, at_time: '10:00' }], '09:30'), 11,
     '★사이가 없으면 같은 수를 주고 시각에 맡긴다(끼우기는 그래도 맞는 답이 나온다)');

  /* ★끌어 옮기는 쪽은 같은 수를 못 쓴다 — 시각이 도로 이기기 때문이다. null 로 말한다 */
  eq(U.seqBetween(10, 30), 20, '가운데 수');
  eq(U.seqBetween(null, 10), 0, '맨 앞으로');
  eq(U.seqBetween(30, null), 40, '맨 뒤로');
  eq(U.seqBetween(null, null), 10, '혼자면 10');
  eq(U.seqBetween(10, 11), null, '★사이가 없으면 null — 지어내지 않고 다시 매기라고 말한다');
  eq(U.seqBetween(10, 12), 11, '  한 칸이라도 남으면 쓴다');
}
...
(async function outbox() {
  ...
  const base = [
    { id: 'a', on_date: '2026-08-29', at_time: '11:00', seq: 20, name: '규카츠' },
    { id: 'b', on_date: '2026-08-29', at_time: null, seq: 30, name: '시각 없는 줄' },
    { id: 'c', on_date: '2026-08-30', at_time: '09:00', seq: 10, name: '이튿날' },
  ];
  const t2 = Outbox.tmpId();
  Outbox.queue({ kind: 'create', tempId: t2, tripId: 'T', row: { on_date: '2026-08-29', at_time: '10:00', seq: 15, name: '새 줄' } });
  ...
  eq(merged.map(r => r.name), ['새 줄', '규카츠', '시각 없는 줄'],
     '★차례(seq)대로 끼어들고 · 지운 줄은 빠지고 · 남의 여행은 안 섞인다');
  /* ★★서버와 **같은 규칙**인지 본다: 차례가 먼저고 시각은 가르는 열쇠일 뿐이다.
     한쪽만 고치면 끊긴 동안 넣은 줄이 연결되는 순간 자리를 옮긴다. */
  /* ⚠ 쌓인 것이 없는 여행으로 묻는다 — 위 묶음이 'T' 에 하나 남겨 두었다 */
  eq(Outbox.apply('Z', [
       { id: 'x', on_date: '2026-08-29', at_time: '22:00', seq: 10 },
       { id: 'y', on_date: '2026-08-29', at_time: '08:00', seq: 20 },
     ]).map(r => r.id), ['x', 'y'],
